...
public class MzmlParser {
    ...
    public List<MsDataPoint> readPartition() throws XMLStreamException, DataFormatException, IOException
    {
        ...
        {
            this.decodeAndCollectData(this.pausedSpecInfo, results, false, this.pauseIndex, this.partitionSize, Double.MAX_VALUE);
        }
        ...
        {    
            ...
            if (this.reader.getEventType() == XMLStreamReader.START_ELEMENT && this.reader.getLocalName().equals("spectrum")) 
            {
                ...
                this.parseSpectrum(results, false, 0, this.partitionSize, Double.MAX_VALUE);
     ...
     * @throws FileNotFoundException 
     */
    public void initPartitionedReadbyRT(double rtStep) throws XMLStreamException, FileNotFoundException
    {
        // retain the partition size
        this.partitionStep = rtStep;
        this.partitionRT = this.msBounds.get()[2] + this.partitionStep;
        System.out.println(this.partitionStep+",0-----------------------------------------"+this.partitionRT);
     ...
     * @throws IOException 
     */
    public List<MsDataPoint> readPartitionbyRT() throws XMLStreamException, DataFormatException, IOException
    {
        // points collection for this partition
        List<MsDataPoint> results = new ArrayList<>();
        ...
        while(this.pausedSpecInfo != null)
        {
            this.decodeAndCollectData(this.pausedSpecInfo, results, false, this.pauseIndex, Integer.MAX_VALUE, this.partitionRT);
            if(this.isPause)
            {
            	this.partitionRT = this.partitionRT + this.partitionStep;
            	this.isPause = false;
            }
        }
        ...
        {    
            ...
            if (this.reader.getEventType() == XMLStreamReader.START_ELEMENT && this.reader.getLocalName().equals("spectrum")) 
            {
                ...
                this.parseSpectrum(results, false, 0, Integer.MAX_VALUE, this.partitionRT);
                
                if(this.isPause)
                {
                	this.partitionRT = this.partitionRT + this.partitionStep;
//                	System.out.println(this.partitionStep+",2222-----------------------------------------"+this.partitionRT);
                	this.isPause = false;
                    return results;
                }
            }
        ...
        this.reader.close();
        this.partitionRT = this.partitionRT + this.partitionStep;
//        System.out.println(this.partitionStep+",3-----------------------------------------"+this.partitionRT);
        return results;
    }
     ...
     * TODO current implementation will return only the contents of the first run element,
     *      extend to handle more than run element
    ...
    private List<MsDataPoint> parseSpectrumList(boolean isCount) throws XMLStreamException, DataFormatException, IOException {
        ...
        while (reader.hasNext()) 
        {
            ...
            {
                this.parseSpectrum(pointResults, isCount, 0, Integer.MAX_VALUE, Double.MAX_VALUE);
            }
     ...
     * @throws IOException 
     */
    private void parseSpectrum(List<MsDataPoint> pointCollection, boolean isCount, int startIndex, int pointLimit, double rtLimit) throws XMLStreamException, DataFormatException, IOException {
...
//                    System.out.println("scanStartTime="+spectrumInfo.scanStartTime);
            ...
            if (this.reader.getEventType() == XMLStreamReader.END_ELEMENT) 
            {
                ...
                if (this.reader.getLocalName().equals("spectrum")) 
                {
                    ...
                    this.decodeAndCollectData(spectrumInfo, pointCollection, isCount, startIndex, pointLimit, rtLimit);
                    return;
                }
     ...
     * @throws IOException 
     */
    private void decodeAndCollectData(SpectrumInformation currentSpecInfo, List<MsDataPoint> pointCollection, boolean isCount, int startIndex, int pointLimit, double rtLimit) throws DataFormatException, IOException 
    {
        ...
        for (int i = startIndex; i < mzArrayDoubles.length; i++) 
        {
            ...
            if(pointCollection.size() == pointLimit || (double)currentSpecInfo.scanStartTime > rtLimit)
            {
                this.pauseIndex = i;
            ...
            if (intensityArrayDoubles[i] >= MIN_INTENSITY_THRESHOLD) 
            {
                ...
                {
                    this.numPoints++;
                    msBounds.update(mzArrayDoubles[i], currentSpecInfo.scanStartTime);
                }
                else
                {
