...
public class MzmlReader {
     ...
     */
    public List<MsDataPoint> getPoints() throws IOException, XMLStreamException, DataFormatException
    {
        ...
        {    
            ...
            if (this.reader.getEventType() == XMLStreamReader.START_ELEMENT && this.reader.getLocalName().equals("run")) 
            {
                ...
                List<MsDataPoint> data = this.parseRun();
     ...
     * @throws IOException 
     */
    private List<MsDataPoint> parseRun() throws XMLStreamException, DataFormatException, IOException {
        ...
        while (this.reader.hasNext()) 
        {
            ...
            if (this.reader.getEventType() == XMLStreamReader.START_ELEMENT && this.reader.getLocalName().equals("spectrumList")) 
            {
                ...
                runPoints = this.parseSpectrumList();
            }
     ...
     * @throws IOException 
     */
    private List<MsDataPoint> parseSpectrumList() throws XMLStreamException, DataFormatException, IOException {
        ...
        while (reader.hasNext()) 
        {
            ...
            {
                this.parseSpectrum(pointResults, 0, Integer.MAX_VALUE);
            }
     ...
     * @throws IOException 
     */
    private void parseSpectrum(List<MsDataPoint> pointCollection, int startIndex, int pointLimit) throws XMLStreamException, DataFormatException, IOException {
        ...
        while (this.reader.hasNext()) 
        {
            ...
            if (this.reader.getEventType() == XMLStreamReader.END_ELEMENT) 
            {
                ...
                {
                	if ((double)spectrumInfo.scanStartTime >= bounds[2] && (double)spectrumInfo.scanStartTime <= bounds[3])
                	{
	                    // decode 64-bit encoded data, collect data points 
	                    this.decodeAndCollectData(spectrumInfo, pointCollection, startIndex, pointLimit);
                	}
                    return;
                }
     ...
     * @throws IOException 
     */
    private void decodeAndCollectData(SpectrumInformation currentSpecInfo, List<MsDataPoint> pointCollection, int startIndex, int pointLimit) throws DataFormatException, IOException 
    {
        ...
        {
        	if (mzArrayDoubles[i] >= bounds[0] && mzArrayDoubles[i] <= bounds[1])
        	{
    ...
    private void updateSpectrumInformation(String[] cvParamPair, SpectrumInformation currentSpecInfo, EncodedData currentEncoding) {
        ...
        {
            case MzmlReader.ACCESSION_SCAN_START_TIME: // "scan start time"
                currentSpecInfo.scanStartTime = Float.parseFloat(cvParamPair[1]);
                break;
        
            case MzmlReader.ACCESSION_MS_LEVEL: // "ms level"
                currentSpecInfo.msLevel = Short.parseShort(cvParamPair[1]);
                break;
            
            case MzmlReader.ACCESSION_MZ_ARRAY: // "m/z array"
                currentEncoding.isMz = true;
                break;
                
            case MzmlReader.ACCESSION_INTENSITY_ARRAY: // "intensity array"
                currentEncoding.isMz = false;
                break;
                
            case MzmlReader.ACCESSION_32_BIT_FLOAT: // "32-bit float"
                currentEncoding.bits = 32;
                break;   
                
            case MzmlReader.ACCESSION_64_BIT_FLOAT: // "64-bit float"
                currentEncoding.bits = 64;
                break;
                
            case MzmlReader.ACCESSION_ZLIB_COMPRESSION: // "zlib compression"
