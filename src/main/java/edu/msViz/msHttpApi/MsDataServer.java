...
public final class MsDataServer {
    ...
    public void setMzTree(MzMD newMD) {
        try {
            updateSaveLock.lock();
            this.mzMD = newMD;
        }
    ...
    private Object getPoints(Request request, Response response){
        ...
        if (mzMD == null || mzMD.getLoadStatus() == ImportState.ImportStatus.NONE) {
            response.status(HttpServletResponse.SC_NO_CONTENT);
            return "No file has been selected.";
        }

        switch (mzMD.getLoadStatus()) {
            ...
            case READY:
                ...
                try{
                    ...
                    m = Integer.parseInt(request.queryParams("m"));
                ...
                List<MsDataPoint> queryResults = mzMD.query(mzmin, mzmax, rtmin, rtmax, m, n);

                // serialize query results as JSON

                StringBuilder queryResultsJSON = JSONify(queryResults,m*n,true);
    ...
    private Object fileStatus(Request request, Response response){
        ...
        switch (mzMD.getLoadStatus()) {
            ...
            case LOADING_MZDB:
                response.status(HttpServletResponse.SC_CONFLICT);
                return mzMD.getLoadStatusString();
            ...
            case READY:
                ...
                double[] bounds = mzMD.getDataBounds();

                // mz min and max
                payload.put("mzmin",bounds[0]); payload.put("mzmax",bounds[1]);

                // rt min and max
                payload.put("rtmin",bounds[2]); payload.put("rtmax",bounds[3]);

//                // int min and max
//                payload.put("intmin",0); payload.put("intmax",0);

                // point count
                try {
                    payload.put("pointcount", mzMD.dataStorage.getPointCount());
                } catch (Exception ex) {
