...
class ExportPanel extends JPanel {
    ...
    public ExportPanel(StartFrame frame) {
        ...
        // ROW 3:  summary infomation
        c.gridy = 3;
        isSummary = new JCheckBox("Summary");
        c.gridx = 1;
        this.add(isSummary, c);
        ...
        nLabel.setHorizontalAlignment(JLabel.RIGHT);
        c.gridx = 0;
        this.add(nLabel, c);
    ...
    {
        double[] bounds = frame.mzMD.getDataBounds();
        if(bounds != null)
        {
    ...
    private void okClicked(ActionEvent e) {
        try
        {
            String s_minMZ, s_maxMZ, s_minRT, s_maxRT, s_m, s_n;
            ...
            if(this.isSummary.isSelected())
            {
                i_m = !s_m.isEmpty() ? Integer.parseInt(s_m) : 0;
                i_n = !s_n.isEmpty() ? Integer.parseInt(s_m) : 0;
            }
            else
            {
            	i_m = 0;
            ...
            if (outputFileResult == JFileChooser.APPROVE_OPTION) 
            {
                ...
                try {
                    int exported = this.frame.mzMD.export(filepath, d_minMZ, d_maxMZ, (float) d_minRT, (float) d_maxRT, i_m, i_n);
